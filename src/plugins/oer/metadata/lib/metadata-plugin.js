...
(function() {
  ...
  define(['aloha', 'aloha/plugin', 'jquery', 'aloha/ephemera', 'semanticblock/semanticblock-plugin', './languages', 'css!metadata/css/metadata-plugin.css'], function(Aloha, Plugin, jQuery, Ephemera, SemanticBlock, languages) {
    var AUTHORS_TEMPLATE, COPYRIGHT_TEMPLATE, DESCRIPTION_TEMPLATE, EDITORS_TEMPLATE, ILLUSTRATORS_TEMPLATE, KEYWORDS_TEMPLATE, LICENCE_TEMPLATE, METADATA_MODAL, METADATA_TEMPLATE, PUBLISHERS_TEMPLATE, SUBJECTS_TEMPLATE, TITLE_TEMPLATE, TRANSLATORS_TEMPLATE, selectors;
    ...
    return Plugin.create('metadata', {
      ...
      $_element: null,
      _showModal: function() {
        var $languageSelect, $modal, key, label, metadata, _i, _len,
          _this = this;
        console.log('asdf');
        $modal = $(METADATA_MODAL);
        $languageSelect = $modal.find('select[name="language"]');
        for (key = _i = 0, _len = languages.length; _i < _len; key = ++_i) {
          label = languages[key];
          $('option').attr('value', key).text(label).appendTo($languageSelect);
        }
        ...
        _.each(metadata.subject, function(subject) {
        ...
        return $modal.find('[data-tab-next]').click(function() {
      ...
      _readMetadata: function() {
        var key, metadata, selector, _i, _len, _results;
        metadata = {};
        _results = [];
        for (key = _i = 0, _len = selectors.length; _i < _len; key = ++_i) {
          selector = selectors[key];
          metadata[key] = [];
          _results.push(this.$_element.find(selector).each(function() {
            return metadata.key.push($(this).text());
          }));
        }
        return _results;
      },
