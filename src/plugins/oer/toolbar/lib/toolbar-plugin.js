  ...
  define(['jquery', 'aloha', 'aloha/plugin', 'PubSub', 'ui/button'], function(jQuery, Aloha, Plugin, PubSub, Button) {
    ...
    makeItemRelay = function(slot) {
      ...
      ItemRelay = (function() {
        ...
        ItemRelay.prototype.show = function() {
          return jQuery(ROOT_SELECTOR).find(".action." + slot).removeClass('hidden');
        };
        ...
        ItemRelay.prototype.setActive = function(bool) {
          if (!bool) {
            jQuery(ROOT_SELECTOR).find(".action." + slot).removeClass('active');
          }
          if (bool) {
            return jQuery(ROOT_SELECTOR).find(".action." + slot).addClass('active');
          }
        };
        ...
        ItemRelay.prototype.setActiveButton = function(subslot) {
          if (subslot) {
            return jQuery(ROOT_SELECTOR).find(".action." + slot + " .subaction." + subslot).addClass('active');
          } else {
            return jQuery(ROOT_SELECTOR).find(".action." + slot + " .subaction." + subslot).removeClass('active');
          }
        };
    ...
    Aloha.bind('aloha-ready', function(event, editable) {
      return jQuery.each(adoptedActions, function(slot, settings) {
        ...
        jQuery(ROOT_SELECTOR).on('click', selector, function(evt) {
          var $target;
          evt.preventDefault();
        ...
        if (settings.preview) {
          jQuery(ROOT_SELECTOR).on('mouseenter', selector, function(evt) {
            var $target;
            $target = jQuery(evt.target);
            if (!($target.is(':disabled') || $target.parent().is('.disabled'))) {
        ...
        if (settings.unpreview) {
          return jQuery(ROOT_SELECTOR).on('mouseleave', selector, function(evt) {
            var $target;
            $target = jQuery(evt.target);
            if (!($target.is(':disabled') || $target.parent().is('.disabled'))) {
    ...
    buildMenu = function(options, selected) {
      ...
      return jQuery(ROOT_SELECTOR).find('.headings .currentHeading').text(formats[selected]);
    };
    ...
    return Plugin.create("toolbar", {
      ...
      init: function() {
        ...
        jQuery(ROOT_SELECTOR).on('mousedown', ".action", function(evt) {
